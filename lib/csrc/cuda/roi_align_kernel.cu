...
torch::Tensor roi_align_forward_cuda(const torch::Tensor& input,
    ...
    const int total_size = num_rois * pooling_height * pooling_width * channels;
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "ROIAlign_forward", [&] {
        RoIAlignForward<scalar_t><<<grid, block>>>(
            total_size,
...
torch::Tensor roi_align_backward_cuda(const torch::Tensor& grad,
    ...
    const int num_rois = grad.size(0);

    const int pooling_width = grad.size(3);
    const int pooling_height = grad.size(2);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad.scalar_type(), "ROIAlign_backward", [&] {
        RoIAlignBackward<scalar_t><<<grid, block>>>(
            grad.numel(),
