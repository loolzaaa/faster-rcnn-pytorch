...
class _RPN(nn.Module):
    ...
    def forward(self, base_feature, im_info, gt_boxes):
        ...
        rpn_cls_score_reshape = rpn_cls_score.view(rpn_cls_score.size(0), 2, -1, _size)
        rpn_cls_prob_reshape = F.softmax(rpn_cls_score_reshape, dim=1)
        rpn_cls_prob = rpn_cls_prob_reshape.view(rpn_cls_prob_reshape.size(0), self.nc_score_out, -1, _size)
